...
def search(map, method=None):
    ...
    method: Either Euclidean or Manhattan
    """
        ...
        for x,y in move:    #For the four directions
            ...
            if pixel != 0: #Check if wall or not
                ...
                if next not in cost_so_far: #If not discovered
                    ...
                    #open.put((new_cost, next)) #Insert into queue

                    #Added A star portion
                    #Check which method to use and find their heuristic cost
                    if (method == "manhattan"):
                        h_cost = manhattan(next, end)   
                    elif (method == "euclidean"):
                        h_cost = euclidean(next, end)

                    #Calculate new priority and add to queue
                    priority = next_cost + h_cost
                    open.put((priority, next)) #Insert into queue
...
if __name__ == "__main__":
    ...
    try: #Get either manhattan or euclidean
        method = str(sys.argv[2])
    except IndexError:
        print("This program requires an additional argument after difficulty.gif: manhattan or euclidean")
        exit(-1)
    ...
    im = im.convert('1')
    search(im.load(), method)
    visualize_search(f"{difficulty[:-4]}A*{method}Solved.png")
